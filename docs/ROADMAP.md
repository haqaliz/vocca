...
| **P0 capture model** | Hold-to-talk default (`⌥Space` down/up), **with a toggle alternative**; no VAD or endpointing | In hold-to-talk the user's finger is the ground truth, which makes "it cut me off mid-sentence" — the #1 rage-quit bug in this category — structurally impossible, and keeps VAD tuning out of P0. Toggle ships alongside it because `PRODUCT_SPEC.md:257` makes it an accessibility requirement, not a preference; it has **no such ground truth**, so it is bounded by the 120 s ceiling, the tap-disabled stop and the system triggers instead. See `docs/planning/audio-capture-hotkey/session-lifecycle/spec.md` for what that costs and what would close it. |
