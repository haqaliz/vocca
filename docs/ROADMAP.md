...
| **P0 capture model** | **Toggle default** (`⌥Space` to start, `⌥Space` to stop), **with hold-to-talk as the alternative**; no VAD or endpointing | Amended 2026-08-25; the two modes swapped roles, neither was removed. Hold-to-talk was the default because the user's finger is the ground truth, which makes "it cut me off mid-sentence" — the #1 rage-quit bug in this category — structurally impossible. That argument still holds and is why the mode stays; what it did not survive was contact with the gesture, where holding a key for a whole utterance produces presses too brief to capture the 0.3 s the engine needs, surfacing as a failure notice. Toggle has **no such ground truth**, so a session is bounded by the 120 s ceiling, the tap-disabled stop and the system triggers instead — the cost is now paid by default rather than by the users who opt in. Toggle was already required regardless: `PRODUCT_SPEC.md` makes it an accessibility need, not a preference. See `docs/planning/audio-capture-hotkey/session-lifecycle/spec.md` for what the missing ground truth costs and what would close it. |
