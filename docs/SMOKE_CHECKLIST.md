...
|---------|------|----------|---------|--------|-----|-------|
| `v0.3.0` | 2026-09-10 | 7 | 3 | 0 | **not computable** | Resumption run (`injection-matrix-completion`, concluded 2026-09-12). **7 of 17 installed deliverable rows recorded, all 7 landing their expected rung** — Notes and Mail re-probed `.accessibility` (window opened 2026-09-10) and it **landed** in both — the demotion restored, promotion observed; Safari, Messages, Firefox, Terminal, Warp landed `.clipboardPaste` (Terminal/Warp run from non-target terminals, provenance founder-reported). Under the memory-ordered FMS definition ratified in this unit (first rung the memory chose = the `attempted:` trace's first element) all 7 recorded rows were also first-method successes. **The unit stopped on a product defect, not a harness defect:** in Chromium/Electron apps (VSCode, Teams, Discord, ChatGPT, Obsidian) the AX focused-app read answers "nothing focused", the ladder refuses at rung 0 with `.noFocusedField` and never reaches clipboardPaste — 5 recovery journals (`~/Library/Application Support/Vocca/recovery/`) + the usage ledger (2026-09-11: 2 delivered / 5 failsafeHeld) are the evidence; no transcript was lost. Escalated as its own unit. Harness changes shipped test-first this unit: the **self-capture guard** (a terminal row run from inside its own target terminal VOIDs; live-proofed `--row Warp` from inside Warp) and the **landing-rung FMS question** (floor 1930 → 1936). *Not yet run:* GoogleDocs; step 92 (Passwords, PasswordField) unexecuted. The Sep-10 rows ran against the master harness (founder ran from the primary checkout); guard-harness re-runs for Terminal/Warp pending. **FMS not computable** — 7 of 17 installed deliverable rows recorded, 7 of 7 on recorded rows (no percentage quoted over the matrix); the ≥19/20 bar remains structurally unreachable on this machine (3 permanent skips → ceiling 17/20). |
| `v0.2.1` | 2026-09-05 | 10 | 3 | 4 | **not computable** | Continuation run (`injection-matrix-completion`, resumed after the unit's early conclusion). **10 distinct deliverable rows attempted, 6 firmly recorded, 4 voided.** *Expected rung landed (4):* TextEdit `.accessibility`; Xcode, Telegram, Chrome `.clipboardPaste`. *Delivered, expected rung missed (2):* Notes and Mail — bytes matched, but `.accessibility` is demoted with a re-probe window to 2026-09-10, so the ladder fell through to clipboard. The demotion-honored outcome, recorded as a miss, not a defect. *Voided (4), each with its reason named:* **Messages, Firefox** — byte mismatch adjudicated to a **harness** defect, neither ASR nor injection: `open -a` returns before a cold-launched application is up and the activation that followed was `|| true`, so the select-all/copy captured whatever was frontmost. Both rows were cold-launched; every row that passed that hour was already running. Fixed test-first in `1985da6` (activation keys on the bundle identifier, polls up to 10 s for the row's app to come frontmost, and VOIDs a capture taken from anything else; RED 24 failures → GREEN). **Terminal, Warp** — voided as *indistinguishable*, not as failures: the harness runs inside a terminal, so for the terminal rows the select-all/copy can capture its own scrollback, which holds the phrase the script just printed. Under the containment semantics added in `386f433` that satisfies the compare, so a pass cannot be told from a self-capture. Re-run both from a terminal that is not the row's target. Asserting a pass whose capture provenance is unknown is the same error as asserting a byte mismatch without knowing the selection was made — this file's first preamble rule, applied symmetrically. *Skipped (3), no same-class swap:* Ghostty, IntelliJ, Zed. *Not yet run (7):* VSCode, Teams, Discord, ChatGPT, Obsidian, Safari, GoogleDocs; **step 92 (Secure Input refusals) also unexecuted.** **FMS not computable:** 4 of 20 deliverable rows landed their expected rung, on 6 firmly recorded rows. Note the bar is **structurally unreachable on this machine** regardless of the remaining rows — with 3 permanent skips the ceiling is 17/20 against a ≥19/20 bar. That is a recorded outcome, not a failure. |
...
| `v0.1.0` | 2026-09-03 | 1 | 3 | 0 | **not closeable** | Baseline run 2 (evidence build, `injection-matrix-record`): run log + `strategies.json` now the file-based evidence chain. 19/22 rows confirmed after three swaps (Pages→Telegram `ru.keepcoder.Telegram`, Notion→ChatGPT `com.openai.codex`, 1Password→Passwords `com.apple.Passwords`); 3 skipped — Ghostty, IntelliJ, Zed not installed, no same-class swap available. **One row run: Notes — byte mismatch (failed); `strategies.json` records the accessibility rung demoted with a fresh re-probe window (2026-09-10) — R1's silent-no-op observed a second time, the ladder fell through. Unified-log session lines for the run window are not verifiable (the live-session check was declined); the row's evidence is the run log + the strategy demotion.** The run is incomplete: 17 rows + step 92 remain; FMS not computable on 1 of 18 deliverable rows. |
