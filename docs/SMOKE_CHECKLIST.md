...
|---------|------|----------|---------|--------|-----|-------|
| `v0.2.1` | 2026-09-05 | 1 | 3 | 0 | **not computable** | Control-row run (`injection-matrix-completion`): the Notes control row re-ran on v0.2.1 with the **evidence chain proven live end to end** — `session opened` + `delivery target=com.apple.Notes rung=clipboardPaste attempted: [clipboardPaste] verified=false` in the unified log, run-log line on disk, `strategies.json` unchanged. **First-execution defect found and fixed test-first: the byte-compare compared raw bytes against the lowercase unpunctuated `PHRASE`, which the engine + rules pipeline's punctuated transcript can never match — every prior row failed `bytes_matched: false` by construction. Fixed with case-fold + terminal-punctuation normalization (`phrase_matches`), pinned by the self-check, driven by CI via `MatrixHarnessSelfCheckTests`.** With the fix: `bytes_matched: true` — the first passing byte-compare in matrix history; the row still misses first-method-success because the log named `clipboardPaste` (accessibility demoted, re-probe 2026-09-10) vs the expected `accessibility` — the demotion-honored outcome, recorded as a miss. **Unit concluded early by founder decision: the remaining 16 deliverable rows + step 92 are NOT executed. FMS not computable (1 of 20 deliverable rows run, and that row missed its expected rung).** The run is resumable at any time — the harness + evidence chain are shipped and each row is ~30 s of dictation. |
