...
| Intent | `IntentResolver` | `KeywordIntentResolver` (token-scored over a seeded synonym table, the not-confident threshold 0.75 → the spoken ask naming the resolver's own top ≤3 candidates, nothing executes; the seed pin makes a retune a reviewed edit) + `PhraseIntentResolver` (exact normalized match over the user's `intent-phrases.json`, never asks; **the composed default since `phrase-intent-resolver`**, 2026-09-25) + `NullIntentResolver` (`.none` for every utterance; the composed default until the N1 flip) — real since `intent-layer` (2026-09-22); **the guardrail-7 claim: two real classifiers, not composed together** (the D3-shaped caveat retired by S1) | **Yes** — a hosted/BYOK classifier is a later *addition* to the seam, never this slice, never a replacement |
...
> *Annotated (`phrase-intent-resolver`, 2026-09-25): the intent row above gains its second
> real classifier, and the composed default flips.* `PhraseIntentResolver` is an exact
> normalized match over the user's `intent-phrases.json` (`IntentPhraseStore` in
