...
/// **The context-wiring family lint** (`bootstrap-wiring` Phase 5, the `KokoroSeamBoundaryTests`
/// shape): the C12 composition's own names are confined to the three files that compose it —
/// the recipe (`VoccaBootstrap/ContextWiring.swift`), the composition root's call
/// (`AppBootstrap.swift`), and the probe drive (`VoccaNetworkProbe/ContextDrive.swift`) —
/// and never the pinned dictation files, the injection ladder, or `VoccaUI` (the
/// `widget-indicator` aspect consumes the root's slots, never the wiring's names).
///
/// The family is the wiring's own vocabulary — `ContextWiring` (the recipe's surface),
/// `ContextRevocation` (the kill's carrier) and `ContextResolutionCarrier` (the per-turn
/// carrier) — a prefix rule, so every member of the family is covered by construction. The
/// never-read guard's second half follows: no `ContextProvider` family name in the two
/// digest-pinned dictation files — "the loop never depends on context" as lint, not
/// discipline (`ContextProvider` is the one name that would make the claim false by
/// construction).
final class ContextWiringSeamBoundaryTests: XCTestCase {

    /// The identifier prefixes that constitute the wiring family: the recipe's surface, the
    /// composition's file-local types, and the recipe's entry point (`composeContextWiring` —
    /// its own prefix, because `\b` cannot see a boundary inside the `compose`-prefixed
    /// identifier).
    private static let wiringFamilyIdentifiers = [
        "ContextWiring", "ContextRevocation", "ContextResolutionCarrier", "composeContextWiring",
    ]
    ...
    /// the composition root names the recipe's call (`composeContextWiring` — the family's
    /// prefix rule covers the `compose`-prefixed form); the probe drive composes the recipe
    /// for the zero-network invariant.
    private static let filesPermittedToNameTheWiringFamily: Set<String> = [
        "VoccaBootstrap/ContextWiring.swift",
        "VoccaBootstrap/AppBootstrap.swift",
        "VoccaNetworkProbe/ContextDrive.swift",
    ]

    /// The identifier prefix of the never-read guard's family: the seam itself.
    private static let providerFamilyIdentifier = "ContextProvider"
    ...
    /// Every `ContextProvider` occurrence in `source`, comments removed first — the never-read
    /// guard's detector.
    ...
    private func sightings(under root: URL) throws -> [String: [String]] {
        ...
        guard !files.isEmpty else {
            throw ContextWiringLintError.noSwiftFilesScanned(under: root.path)
        }
    ...
    func testOnlyTheThreeCompositionFilesMayNameTheContextWiringFamily() throws {
        let root = try sourcesRoot()
        guard FileManager.default.fileExists(atPath: root.path) else {
            throw ContextWiringLintError.sourcesDirectoryMissing(expectedAt: root.path)
        }
        ...
        XCTAssertTrue(
            offenders.isEmpty,
            "the context-wiring family is named outside the three composition files: "
                + "\(offenders.sorted())")
    ...
    func testTheLintDetectsAPlantedContextWiringUse() {
        let source = """
            import VoccaCore

            public struct Leak {
                public var wiring: ContextWiring? { nil }
                public var revocation = ContextRevocation()
                public var carrier: ContextResolutionCarrier? { nil }
                public func composeContextWiring() {}
            }
            """
        let identifiers = Self.wiringFamilyIdentifiers(inSource: source)
        XCTAssertEqual(
            identifiers,
            ["ContextWiring", "ContextRevocation", "ContextResolutionCarrier",
                "composeContextWiring"],
    ...
    func testADocCommentNamingTheContextWiringDoesNotTripTheLint() {
        let source = """
            /// The one file in `Sources/VoccaBootstrap` permitted to name ContextWiring and
            /// its file-local types — the recipe holds the composition, and everything decided
            /// about it lives above it.
            import VoccaCore
            """
        let identifiers = Self.wiringFamilyIdentifiers(inSource: source)
    ...
    /// **No `ContextProvider` family name in the pinned dictation path** — "the loop never
    /// depends on context" as lint, not discipline: the two digest-pinned files name no
    /// member of the seam's family, so a future context dependency there cannot compile into
    /// the dictation path silently. (`ContextWiring` reaching these files would already trip
    /// the wiring-family scan above; this is the seam's own family, guarding the boundary
    /// from the other direction.)
    func testThePinnedDictationFilesNeverNameTheContextProviderFamily() throws {
        ...
        for relative in Self.pinnedDictationFiles {
            ...
            XCTAssertTrue(
                identifiers.isEmpty,
                "\(relative) names the ContextProvider family: \(identifiers) — the dictation "
                    + "path must never depend on context (D2)")
        }
    }
}

/// The lint's own failure vocabulary.
private enum ContextWiringLintError: Error {
