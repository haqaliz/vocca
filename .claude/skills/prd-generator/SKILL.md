...
This skill supports planning at initiative scope — features, epics, or projects spanning one or more cycles. In the `vocca-begin-fast` pipeline it is Phase 4: refine and self-critique the `prd.md` that `prd-interview` produced, then surface the gaps before the review gate.
...
8. **Scope & Layer Fit** *(Vocca-specific)* — Does the work stay in scope (macOS-only, local-first, dictation-first, open-core)? Does it name which layer it touches (capture / ASR / AI-cleanup / injection / TTS-voice-loop / actions) and its phase (P0..P5)? Does it keep audio and text on-device by default and ASR/TTS/LLM pluggable behind interfaces, rather than putting cloud in the OSS core or crippling the local core to set up the paid tier? Does it respect the latency and injection-reliability battles?
...
Problem Statement → Goals & Success Metrics → Users & Scenarios → Requirements (must/should/nice) → Technical Considerations (incl. latency budget, injection reliability, privacy/local-first, which layer + phase) → Risks & Mitigations → Out of Scope. The Lightweight Brief collapses these to one-liners; the Full Spec adds Data Model, latency/privacy budgets, and Non-Functional Requirements.

### Step 3 — Generate the PRD

Write it following the chosen depth. Always include: explicit assumptions (labeled as such), open questions that still need answers (don't paper over gaps), and a "Risks & Mitigations" section (never skip this). Reference a roadmap risk where a named one applies, and name the phase (P0..P5 in `docs/ROADMAP.md`) the work belongs to.

### Step 4 — Self-critique

After generating, run the Critique workflow (Workflow 1) against your own output. Flag any 🔴 or 🟡 areas and note them at the end as "Areas to strengthen before sharing." In the `vocca-begin-fast` pipeline these flagged gaps are exactly what you present at the ⛔ review gate.
...
├─ Technical ......... {latency/injection/privacy in a phrase}
├─ Risks ............. {top risk}
...
6. **Never wave through scope drift** — if a PRD puts cloud in the OSS core, crosses to another platform before macOS is solid, or cripples the local core to set up the paid tier, flag it against the `CLAUDE.md` guardrails before approving.
7. **Never wave through unmeasured claims or ignored UX battles** — if a PRD claims latency/accuracy it hasn't measured, over-scopes the assistant before dictation is excellent, or hand-waves the injection-reliability and latency battles, that's a 🔴 regardless of how good the rest is.
